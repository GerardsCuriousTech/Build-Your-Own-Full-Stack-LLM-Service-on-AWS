...
This is a GitBook-synced documentation repository, not an application. There is no `cargo`, no unit tests, and no performance tests. "Build & test" means the content-quality validators described in `AGENTS.md` (grep-gate, JSON validation, contract validation, link check, markdownlint) — run whichever already exist on the branch; `.github/workflows/ci.yml` becomes the authoritative gate once item 17 lands.
...
Create `.gitbook.yaml` (`root: docs/`). Move every existing course module folder, `SUMMARY.md`, and the course `README.md` into `docs/`, fixing all internal links. Rename `project-sec-cik-lookup-module` → `docs/project-sec-cik-lookup` and `optional-front-end-module-with-aws-amplify` → `docs/partner-bot-web-page`. Write the full target `docs/SUMMARY.md` nav tree (part groups: Foundations, Cloud Deployment — SAM, Cloud Deployment — CDK, Full-Stack Integration, Advanced Modules, Reference) with non-blank placeholder READMEs for the new `cdk-bridge/`, `mcp-module/`, `langchain-and-rag/` modules and placeholder `docs/reference/` pages. Add repo-root `README.md` (repo purpose, distinct from the course intro), `course-content-audit.md`, `tasks.md`, `docs-map.md`, and `freshness-checklist.md`. Land this atomically — a half-moved state with content under `docs/` but no `.gitbook.yaml`/`SUMMARY.md` breaks the published book.
...
Rewrite `docs/project-10q-inference/README.md` for CDK-based deployment. Write `part-1-inference-test.md` (fold in the old `sample-prompt-without-context.md`, then delete it) and `part-2-inference-with-context.md` (date-relative 10-Q filing selection; fold in the old `sample-prompt-with-context.md`, then delete it). Follow the pedagogy template; link to `reference/models.md` and `reference/contract.md`. Consult `course-content-audit.md` per-page notes.
...
Write `docs/project-10q-inference/part-3-text-extraction.md` (extracting text from SEC filing documents) and rewrite `docs/project-10q-inference/part-4-question-to-enhanced-prompt.md` with Lambda I/O aligned to `reference/contract.md`. Both follow the pedagogy template and use CDK for deployment.
...
Write `docs/partner-bot-web-page/connect-to-lambda.md` (API Gateway integration with the existing Lambda; reference `reference/contract.md` for request/response format) and `docs/partner-bot-web-page/authentication-with-cognito.md` (Cognito auth via Amplify Gen 2). Both follow the pedagogy template.
...
Write `scripts/validate-contract.py` (load the schema from `reference/contract.md`'s JSON blocks, scan project pages for Lambda request/response JSON, validate against the schema). Write `.markdownlint.yml` (GitBook-flavored rules plus enforcement that project pages contain the six pedagogy-template headings). Write `.github/workflows/ci.yml` wiring checkout → markdownlint → grep-gate → JSON validation → link check (lychee) → contract validation, triggered on push to `main` and PRs targeting `main`. This becomes the authoritative quality gate.
